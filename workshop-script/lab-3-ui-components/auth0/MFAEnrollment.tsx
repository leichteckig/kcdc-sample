...
export default function MFAEnrollment({
  ...
  const handleCreateEnrollment = (factor: string) => async () => {
    ...
    if (response.status !== 200) {
      ...
      return toast("There was a problem starting the enrollment process. Try again later.");
    }
      ...
      <Card>
        ...
        <CardContent className="grid gap-6 p-4 pt-0 md:p-6 md:pt-0">
          ...
          {currentFactors &&
            ...
            !fetching &&
            currentFactors
              .filter((factor: MfaEnrollment) => factor.enabled)
              .map((factor: MfaEnrollment, idx: number) => {
